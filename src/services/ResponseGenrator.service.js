...
class ResponseGenratorService {
    ...
    getSearchAndSortConditions(condition, subOrQuery) {
        ...
        if (this.search) {
            if (this.searchable && Array.isArray(this.searchable) && this.searchable.length > 1) {
                searchCondition["$or"] = this.searchable.map((searchable) => {
                    return { [searchable]: { $regex: '.*' + this.search + '.*', $options: 'i' }, ...subOrQuery }
                })
            } else {
                searchCondition[this.searchable[0]] = { $regex: '.*' + this.search + '.*', $options: 'i' }
    ...
    async getAggrigatedPaginatedResponse(query) {
        return await Promise.all([
